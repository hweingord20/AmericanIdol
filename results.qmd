...
ggplot(finalists_with_season, aes(x = age, color = group, fill = group)) + 
  ...
  labs( title = "Age Distribution: Finalists vs Winners", x = "Age", y = "Density" ) + scale_color_manual(values = c("Finalist" = "steelblue", "Winner" = "darkred")) + scale_fill_manual(values = c("Finalist" = "steelblue", "Winner" = "darkred")) +
  theme_minimal()
```
```{r}
...
ggplot_object <- ggplot(auditions_filtered, aes(x = tickets_to_hollywood, y = reorder(audition_city, tickets_to_hollywood, FUN = function(x) length(x)))) + 
  ...
  theme(strip.text = element_text(size = 10, face = "bold"),
        axis.text.y = element_text(size = 5, margin = margin(r = 10), lineheight = 1.2), 
        axis.text.x = element_text(size = 9) ) 

ggplotly(ggplot_object, height = 1000)
